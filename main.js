...
import { getFirestore, collection, doc, getDocs, addDoc, updateDoc, onSnapshot, deleteDoc } from "firebase/firestore";
...
// keep a map of unsubscribers for each service's queue listener
const queueUnsubs = {};
...
function listenToServices(){
  ...
  onSnapshot(servicesRef, (snapshot) => {
    ...
    snapshot.forEach((docSnap) => {
      ...
      // keep any existing queue we already loaded
      const existing = services.find(s => s.id === id);
      next.push({
        id,
        name: d.name || "Unnamed service",
        avgMinsPerPerson: Number(d.avgMinsPerPerson) || 5,
        open: d.open !== false,
        queue: existing?.queue || []  // filled by sub-listener below
      });

      // attach a listener to this service's /queue if not attached yet
      if (!queueUnsubs[id]) {
        const qRef = collection(db, "services", id, "queue");
        queueUnsubs[id] = onSnapshot(qRef, (qSnap) => {
          const arr = [];
          qSnap.forEach((qDoc) => arr.push(qDoc.data()));
          // update just this service's queue
          const idx = services.findIndex(s => s.id === id);
          if (idx >= 0) {
            services[idx] = { ...services[idx], queue: arr };
          } else {
            // if services hasn't been set yet, push and let the next render normalize
            services.push({ id, name: d.name, avgMinsPerPerson: d.avgMinsPerPerson, open: d.open, queue: arr });
          }
          renderServiceList();
          renderDetails();
          renderMyTicket();
    ...
    // cleanup listeners for services that no longer exist
    Object.keys(queueUnsubs).forEach(id => {
      if (!seenIds.has(id)) {
        queueUnsubs[id](); // unsubscribe
        delete queueUnsubs[id];
      }
    });
...
async function joinQueueFirestore(svc, name){
  ...
  joining = true;
  try {
  console.log("Joining queue for", svc, name, "with name:", name);
  const queueRef = collection(db, "services", svc.id, "queue");
  const qSnap = await getDocs(queueRef);

  const newTicketId = (svc.name.split(" ")[0][0] + String(svc.queue.length+1).padStart(2,"0")).toUpperCase();
...
async function serveNextFirestore(svc){
  console.log("Serving next ticket for:", svc);
  const qRef = collection(db, "services", svc.id, "queue");
  const docs = await getDocs(qRef);

  if (docs.empty) {console.log("Queue is empty"); return; } 

  const firstDoc = docs.docs[0];
  await deleteDoc(firstDoc.ref);
  console.log("Removed first ticket:", firstDoc.data());

}

// Remove person from queue (admin)
async function removeFromQueueFirestore(svc, ticketId){
  const qRef = collection(db, "services", svc.id, "queue");
  const docs = await getDocs(qRef);


  docs.forEach(d=>{
    if (d.data().id === ticketId){
      deleteDoc(d.ref);
    }
    });
}
...
// Toggle open/close
async function toggleQueueFirestore(svc){
  const svcRef = doc(db, "services", svc.id);
  await updateDoc(svcRef, { open: !svc.open });
}
// ----- Citizen View -----
...
function renderAdminDetails(){
  ...
  toggleBtn.onclick = async () => { await toggleQueueFirestore(svc); };

  content.className = "";
  content.innerHTML = "";

  const btnServe = el("button", {
    class: "btn secondary",
    text: "Serve Next",
    onclick: async () => { await serveNextFirestore(svc); }
  });
    ...
    list.appendChild(
      el("li", {}, [
        ...
        el("button", {
          class: "btn",
          text: "Remove",
          onclick: async () => { await removeFromQueueFirestore(svc, q.id); }
        })
      ])
    );
  });
